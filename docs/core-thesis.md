# The Framingo Project 設計憲章（改訂版・中核仕様）
...
| **媒体** | 両者を繋ぐ表現形式 | Framingo（設計された形式言語） |
...
```
    ...
    │  ・接地制約下でのみ出力                    │
    └───────────────┬────────────────┘
                    │  Function Call（Framingo形式のクエリ）
    ┌───────────────┴────────────────┐
    │      知識供給層（Knowledge Layer）         │
    │  ・検索／DB／API／センサ／シミュレータ     │
    │  ・結果を Framingo に正規化して返す        │
    └────────────────────────────────┘
```
...
| NL⇄Framingo 対訳データ | LLM | 照合・人手 | ここに局在 |
