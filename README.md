...
| `knowledge.py` | The knowledge layer: what a query asks and what comes back / 知識供給層。問いが訊くものと、返ってくるもの | A query is built from an action structurally, so forming one is an operation rather than an act of knowing. Matching reuses the checker's own chaining, so the store and the verifier cannot drift apart / 問いは行為から構文的に組まれるので、組むことは知ることではなく操作である。照合は検証器自身の連鎖を使うので、店と検証器は食い違えない |
...
| `basics.py` | Basic information about basic vocabulary: many names, each met rarely, every consequence stated about the class / 基本的語彙の基本的情報。名前は多く、一つあたりは稀にしか現れず、帰結はすべてクラスについて述べる | The core mentions no name at all — 42 rules where the same physics needed 440 per name. A name is known only by looking it up / 核は名前を一つも含まない。同じ物理が名前ごとなら440規則だったところが42規則。名前は引いてしか分からない |
| `instantiation.py` | A corpus with no physics in it: only the act of applying a rule / 物理のないコーパス。規則を適用する行為だけ | Kept as a narrow component test — can retrieval deliver knowledge a model has never seen — and **not** as a test of the central claim; calling it that was a mistake / 「取得は未見の知識を届けられるか」という狭い部品検査として保持する。中心主張の試験ではない |
| `experiments/train.py` | Trains a ~0.6M-parameter Transformer from scratch and grades it with the verifier / ゼロから約60万パラメータの Transformer を訓練し、検証器で採点する | Reports accuracy, grounding rate, detection of fabrications, and — where the model fetches anything — what it scores when the answer is made to lie / 正答率、接地率、捏造の検出率、そして取得を伴う場合は、答えに嘘をつかせたときの点数を報告する |
| `experiments/adversary.py` | Mutates the world's own gold meanings across classes of wrong recombination / 世界の gold 意味を、誤結合のクラスにわたって変異させる | Measures the check over a space of errors rather than the sample six weak models happened to produce / 検査を、6つの弱いモデルがたまたま出した標本ではなく、誤りの空間に対して測る |
| `experiments/ablate_core.py` | Removes RULEs from the core and re-grades saved predictions / 核から RULE を取り除き、保存済み予測を採点し直す | No training needed. Ground truth is computed against the world, never against the ablated core / 訓練不要。真値は世界に対して計算し、削った核に対しては計算しない |
...
法則ではない —— そしてこれはその後、仮定ではなく測定になった。同じ物理を名前ごとの
列挙ではなくクラス水準の一般規則として書くと、1規則を削る損害は **0.15 ポイントに
対して 3.85 ポイント**であり、どの規則を落とすかによる差もはるかに大きくなる。
**よく書かれた核とは、内部の冗長性が少ない核である。** ゆえにそこに空く穴の代償は
大きい。この核を440規則から42規則へ圧縮したことで、読めるようになり、速くなり、
一度も見ていない名前にも届くようになったが、同時に第9章の網羅性の問題は易しくでは
なく**難しく**なった。
...
1. **Coverage of the minimal core** — the boundary need not be exact, but "absent from both core and retrieval" remains a real gap, and the cost of one has been measured: rewriting the same physics as class-level rules rather than one rule per name raises what a missing rule costs by a factor of twenty-five. **Writing the core better removes the redundancy it had inside itself**, so this gets harder as the core improves. / 最小核の網羅性——境界の確定は不要だが、「核にも外部にも無い」は残り、その代償が測れた。同じ物理を名前ごとではなくクラス水準で書くと、規則一つの欠落の代償が25倍になる。**核をよく書くほど内部の冗長性は減る**ので、この問題は核が良くなるほど難しくなる
2. **Design of the translation boundary** — the only place hallucination can enter, and now in two layers rather than one: natural language into Framingo, and a word outside the instinct vocabulary into words inside it. Localising it further is progress; solving it is untouched. / 翻訳境界の設計——幻覚が入りうる唯一の場所。いまは一層ではなく二層になった。自然言語から Framingo へ、そして本能語彙の外の語から内の語へ。局在がさらに進んだのは前進だが、解決には手が付いていない
3. **What has to be held, and what can be fetched.** The old form of this — "can logic be learned without knowledge?" — was the wrong question: chapter 3 of the charter says a core with no axioms about how the world behaves leaves nothing standing. The sharper one is where the line runs, and the answer taken here is that **the arbitrary part is fetched and the general part is held**. Measured against size, it survives: at 160 classes and 486 core rules a 0.66M-parameter model is still at 0.996, and so, inconveniently, is the arm that fetches nothing. **What that says is that neither the world nor the model is yet the binding constraint**, so the parameter saving the architecture is supposed to buy has no number attached to it. Shrinking the model to 157k parts them — 0.99 against 0.37 — and that is not the number either: the same arm reaches 0.994 given three times the epochs, so what was measured is how fast each route is learned under one budget and not how much either needs to hold. Stated that way it does predict something, and the prediction holds. Hold the budget and grow the world from 20 classes to 320, fifteen times the rules: the arm that fetches goes 1.000 → 0.922 and the arm that memorises goes 0.998 → 0.142. **The gap is with the world's size, and it is a claim about learning rather than about parameters.** How many epochs the memorising arm needs at 320 classes, and whether that number grows faster than the world does, is the open part. / **何を保持し、何を取得するか。** 旧来の形 ——「知識なしに論理は学べるか」—— は問いが誤っていた。憲章第3章は、世界の振る舞いについての公理を持たない核では何も成立しないと述べている。鋭い問いは線がどこを通るかであり、ここで採った答えは**恣意的な部分を取得し、一般的な部分を保持する**である。規模に対しては保つ。クラス160・核486規則で、66万パラメータのモデルは 0.996 を出す。**そして都合の悪いことに、何も取得しないアームも同じ数字を出す。** つまり**世界もモデルも、まだ律速ではない。** このアーキテクチャが買うはずのパラメータ節約には、**まだ数字が付いていない。** モデルを 15.7万まで縮めると両者は割れる（0.99 対 0.37）が、**それも数字ではない** —— 同じアームは epoch を3倍与えれば 0.994 に達する。測れたのは「一つの予算の下でどちらの経路が速く学ばれるか」であって、「どちらがどれだけ保持を要するか」ではない。**そう述べれば予測を持ち、予測は当たる。** 予算を固定してクラスを 20 から 320 へ、規則を15倍にすると、**引く経路は 1.000 → 0.922、覚える経路は 0.998 → 0.142。** **差は世界の大きさとともに開く。これは学習についての主張であって、パラメータについての主張ではない。** クラス320 で覚える経路が何 epoch を要するか、そしてその数が世界より速く増えるかが、残っている部分である
4. **Query bootstrapping** — knowing *what to ask for*. Half of it was never a problem: a word outside the instinct vocabulary carries a mark, so a parser fetches it before the model sees anything and no judgement is needed. The other half is asking for what is *not there* — "name a marsupial other than a kangaroo" — which only becomes visible after reasoning and can be read off no shape, there being no word. It now has a world (`vessels.py`), and the language needed nothing added: `Any.Vessel.Thing` in a consequent is the spec's own way of saying *an individual is required here and the rule cannot say which*. A model asks for the right class 1.000 of the time against 1/3 by chance, uses what comes back, and names nothing it was not given. **What remains open is what it should do when the answer is that there is none.** Given an empty store it asks a second time and then invents — 1.000, every example — and the verifier refuses every one, which is the constraint working and not the model behaving. / クエリ発行のブートストラップ——**何を訊くべきか**。半分ははじめから問題ではなかった。本能語彙の外の語には印が付くので、パーサーがモデルより先に引き、判断は要らない。残る半分は**そこに無いもの**を訊くこと ——「カンガルー以外の有袋類を挙げよ」—— であり、これは推論して初めて見え、**語が無い以上どんな形からも読み取れない。** これに世界が付いた（`vessels.py`）。**言語に足したものは無い。** 帰結に置かれた `Any.Vessel.Thing` が、仕様自身の「ここに個体が要る。どれかは規則が言えない」である。モデルは正しいクラスを 1.000 で訊き（偶然なら 1/3）、返ってきたものを使い、渡されていないものを名指さない。**開いているのは「無い」が答えだったときどうするかである。** 店を空にすると、**もう一度訊き、それから捏造する** —— 1.000、全例で。そして検証器が全件却下する。**これは制約が働いているのであって、モデルが行儀よくしているのではない**
5. **No dialogue layer yet** — no speaker/hearer, no speech acts, no cross-turn anaphora. And a new obstacle: marked words are renumbered per example so that nothing can be learnt about any particular name, which means **a name's identity does not survive the example it appears in**. Anything that tracks the same individual across turns has to undo that. / 対話層が存在しない。そして新しい障害が一つ。印付きの語は例ごとに振り直されるので、特定の名前について何も学べない代わりに、**名前の同一性が例を跨がない**。ターンを跨いで同じ個体を追うものは、これを解かねばならない
6. **The verifier is only as right as the core it reads** — every number above depends on it, and its defects have been found by measuring rather than by reading output: a core that derived the copy error it was meant to catch, a `->` read as a list separator, class membership that was never inferred. / 検証器は、それが読む核が正しい限りでしか正しくない——上記のすべてがこれに依存しており、その欠陥は出力を読むのではなく測ることによって見つかってきた。捕まえるべきコピー誤りを自分で導出していた核、列の区切りとして読まれていた `->`、一度も推論されなかったクラス所属
