...
3. **What has to be held, and what can be fetched.** The old form of this — "can logic be learned without knowledge?" — was the wrong question: chapter 3 of the charter says a core with no axioms about how the world behaves leaves nothing standing. The sharper one is where the line runs, and the answer taken here is that **the arbitrary part is fetched and the general part is held**. It works at eleven classes. Whether it survives a vocabulary the size of Lojban's gismu inventory is untested. / **何を保持し、何を取得するか。** 旧来の形 ——「知識なしに論理は学べるか」—— は問いが誤っていた。憲章第3章は、世界の振る舞いについての公理を持たない核では何も成立しないと述べている。鋭い問いは線がどこを通るかであり、ここで採った答えは**恣意的な部分を取得し、一般的な部分を保持する**である。クラス11では成立する。Lojban の gismu 目録ほどの語彙で保つかは未検証
4. **Query bootstrapping** — knowing *what to ask for*. Half of it is gone: a word outside the instinct vocabulary carries a mark, so a parser fetches it before the model sees anything and no judgement is needed. The half that remains is asking for what is *not there* — "name a marsupial other than a kangaroo" — which only becomes visible after reasoning and cannot be read off any shape. / クエリ発行のブートストラップ——**何を訊くべきか**。半分は消えた。本能語彙の外の語には印が付くので、パーサーがモデルより先に引き、判断は要らない。残る半分は**そこに無いもの**を訊くこと ——「カンガルー以外の有袋類を挙げよ」—— であり、これは推論して初めて見え、どんな形からも読み取れない
